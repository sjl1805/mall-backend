...
public interface UserBehaviorMapper extends BaseMapper<UserBehavior> {
     ...
     */
    @Select("<script>" +
            "SELECT DATE_FORMAT(behavior_time, #{pattern}) AS time_segment, " +
    ...
    List<BehaviorStatsDTO> analyzeBehavior(@Param("query") BehaviorAnalysisQueryDTO query, 
                                          @Param("pattern") String datePattern);

    /**
     * 获取用户最近浏览记录
     */
    @Select("SELECT ub.*, p.name as product_name, p.image as product_image " +
            "FROM user_behavior ub " +
            "LEFT JOIN product p ON ub.product_id = p.id " +
    ...
    BehaviorStatsDTO selectProductBehaviorStats(@Param("productId") Long productId);
